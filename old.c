...
/*   Updated: 2024/02/16 17:54:53 by rseelaen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	dead_check(t_super *super)
{
	pthread_mutex_lock(super->dead);
	if (super->dead_flag == TRUE)
	{
		pthread_mutex_unlock(super->dead);
		return (1);
	}
	pthread_mutex_unlock(super->dead);
	return (0);
...
void	*supervisor(void *main)
{
	...
	while (m->super->dead_flag == FALSE)
	{
		...
		while (i < m->nbr_of_philos)
		{
			...
			{
				pthread_mutex_lock(m->super->dead);
				m->super->dead_flag = TRUE;
				pthread_mutex_unlock(m->super->dead);
...
int	eat(t_philo *p)
{
	...
	pthread_mutex_lock(p->super->general);
	p->last_meal = get_cur_time();
	pthread_mutex_unlock(p->super->general);
...
void	*routine(void *philo)
{
	...
	p = (t_philo *)philo;
	if (p->id % 2 == 0)
		usleep((p->time_to_sleep * 1000) / 2);
	meals = 0;
	p->last_meal = get_cur_time();
	while (!dead_check(p->super) && (p->nbr_of_meals == -1
			|| meals < p->nbr_of_meals))
	{
		if (eat(p))
			break ;
		meals++;
		if (dead_check(p->super) || (p->nbr_of_meals != -1
				&& meals >= p->nbr_of_meals))
		{
			p->state = FULL;
			break ;
		...
		{
			pthread_mutex_lock(p->super->dead);
			p->super->dead_flag = TRUE;
			pthread_mutex_unlock(p->super->dead);
